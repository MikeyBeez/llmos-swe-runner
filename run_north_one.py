...
Design points:
  - The passing set is built from ALL results history (runs/*/results*.json,
    ...
    resolved=true at least once. Frozen to runs/laguna/one_set.json on
...
writes ~/swe/runs/laguna/one1.json
"""
...
SETF = os.path.join(LDIR, "one_set.json")
OUT = os.path.join(LDIR, "one1.json")
MAX_ATTEMPTS = 1
