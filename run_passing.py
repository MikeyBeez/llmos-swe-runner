...
Design points:
  - The passing set is built from ALL results history (runs/*/results*.json,
    ...
    resolved=true at least once. Frozen to runs/laguna/passing_set.json on
...
writes ~/swe/runs/laguna/regress1.json
"""
import json, os, sys, glob, time, socket, subprocess

SWE = os.path.expanduser("~/swe")
LDIR = os.path.join(SWE, "runs", "laguna")
os.makedirs(LDIR, exist_ok=True)
SETF = os.path.join(LDIR, "passing_set.json")
OUT = os.path.join(LDIR, "regress1.json")
MAX_ATTEMPTS = 1
MODEL_TAG = "laguna-xs-2.1-q4km"
