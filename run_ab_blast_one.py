"""One BLAST_RADIUS A/B run: run_ab_blast_one.py <cond:on|off> <instance_id> <rep>.
Varies BLAST_RADIUS only (standard harness otherwise). Records outcome + tool-mix
+ whether the agent CALLED neighbor_tests and whether it saw a regression, to
runs/ab_blast/results.jsonl."""
import json, os, sys, time, socket
SWE = os.path.expanduser("~/swe")
cond, iid, rep = sys.argv[1], sys.argv[2], sys.argv[3]
ABDIR = os.path.join(SWE, "runs", "ab_blast")
os.makedirs(os.path.join(ABDIR, "ev"), exist_ok=True)
evpath = os.path.join(ABDIR, "ev", "%s_%s_%s.jsonl" % (cond, iid, rep))

os.environ["BLAST_RADIUS"] = "1" if cond == "on" else "0"
os.environ["ISOLATE_DISCIPLINE"] = "0"
os.environ["STUCK_ESCALATE"] = "0"
...
insts = {i["instance_id"]: i for i in json.load(open(os.path.join(SWE, "instances_full300.json")))}
...
turns = reads = exps = patches = nbr_calls = nbr_regr_seen = 0
pend = None
try:
    for line in open(evpath):
        try: e = json.loads(line)
        except Exception: continue
        if e.get("phase") != "fix": continue
        t = e.get("type")
        if t == "generation":
            turns += 1
        elif t == "tool_call":
            pend = e.get("tool")
            if pend in ("read_range", "locate"): reads += 1
            elif pend in ("check", "reproduce"): exps += 1
            elif pend == "patch": patches += 1
            elif pend == "neighbor_tests": nbr_calls += 1
        elif t == "tool_result" and pend == "neighbor_tests":
            res = e.get("result") or {}
            rg = res.get("regressed")
            if rg and rg != 0: nbr_regr_seen += 1
except Exception:
    pass

rec = {"cond": cond, "id": iid, "rep": rep, "resolved": bool(r.get("resolved")),
       "secs": secs, "fix_turns": turns, "reads": reads, "exps": exps,
       "patches": patches, "neighbor_calls": nbr_calls, "neighbor_regr_seen": nbr_regr_seen}
open(os.path.join(ABDIR, "results.jsonl"), "a").write(json.dumps(rec) + "\n")
print("AB_BLAST %-3s %-24s rep%s -> %-4s turns=%d patch=%d nbr_calls=%d nbr_regr=%d secs=%d"
      % (cond, iid, rep, "PASS" if rec["resolved"] else "miss",
         turns, patches, nbr_calls, nbr_regr_seen, secs), flush=True)
