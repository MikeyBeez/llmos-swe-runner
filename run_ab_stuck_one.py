"""One stuck-breaker A/B run: run_ab_stuck_one.py <cond:on|off> <instance_id> <rep>.
Varies STUCK_ESCALATE (the only variable). Holds ISOLATE_DISCIPLINE OFF (standard
harness) and TRUNC_RETRY ON in BOTH arms, so the truncation fix helps both equally
and STUCK is cleanly isolated. Records outcome + turns + tool-mix + stuck-fire count
+ truncation-growths to runs/ab_stuck/results.jsonl.
"""
import json, os, sys, time, socket

SWE = os.path.expanduser("~/swe")
cond, iid, rep = sys.argv[1], sys.argv[2], sys.argv[3]
ABDIR = os.path.join(SWE, "runs", "ab_stuck")
os.makedirs(os.path.join(ABDIR, "ev"), exist_ok=True)
evpath = os.path.join(ABDIR, "ev", "%s_%s_%s.jsonl" % (cond, iid, rep))

os.environ["STUCK_ESCALATE"] = "1" if cond == "on" else "0"
os.environ["ISOLATE_DISCIPLINE"] = "0"     # standard harness, held constant
os.environ["TRUNC_RETRY"] = "1"            # truncation fix ON in both arms
...
turns = reads = exps = patches = stuck_fires = trunc_grows = 0
pend = None
try:
    for line in open(evpath):
        ...
        if t == "generation":
            turns += 1
            if (e.get("trunc_grow") or 0) > 0:
                trunc_grows += 1
        elif t == "tool_call":
            pend = e.get("tool")
            if pend in ("read_range", "locate"):
                reads += 1
            elif pend in ("check", "reproduce"):
                exps += 1
            elif pend == "patch":
                patches += 1
        elif t == "tool_result" and pend == "patch":
            err = str((e.get("result") or {}).get("error") or "")
            if err.startswith("NO PROGRESS") or err.startswith("PATCH LOCKED"):
                stuck_fires += 1
except Exception:
    pass
...
rec = {"cond": cond, "id": iid, "rep": rep, "resolved": bool(r.get("resolved")),
       ...
       "patches": patches, "stuck_fires": stuck_fires, "trunc_grows": trunc_grows}
open(os.path.join(ABDIR, "results.jsonl"), "a").write(json.dumps(rec) + "\n")
print("AB_STUCK %-3s %-24s rep%s -> %-4s turns=%d reads=%d exps=%d patch=%d "
      "stuck=%d trunc=%d secs=%d"
      % (cond, iid, rep, "PASS" if rec["resolved"] else "miss",
         turns, reads, exps, patches, stuck_fires, trunc_grows, secs), flush=True)
