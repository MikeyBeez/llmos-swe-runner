...
Design points:
  - The passing set is built from ALL results history (runs/*/results*.json,
    ...
    resolved=true at least once. Frozen to runs/laguna/miss_set.json on
...
writes ~/swe/runs/laguna/misses1.json
"""
import json, os, sys, glob, time, socket, subprocess

SWE = os.path.expanduser("~/swe")
LDIR = os.path.join(SWE, "runs", "north")
os.makedirs(LDIR, exist_ok=True)
SETF = os.path.join(LDIR, "miss_set.json")
OUT = os.path.join(LDIR, "misses1.json")
MAX_ATTEMPTS = 2
MODEL_TAG = "north-mini-code-1.0-ud-iq3s"
